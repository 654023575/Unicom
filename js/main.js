...
angular.module('app').controller('AppCtrl', [ '$idle', '$state', '$scope', '$translate', '$localStorage', '$window', 'subject',
		'usernamePasswordToken', 'dialogs', 'toaster', 'Restangular',
		function($idle, $state, $scope, $translate, $localStorage, $window, subject, usernamePasswordToken, dialogs, toaster, Restangular) {
			...
			$scope.$on('$userTimeout', function() {
				...
				/*
				 * var dlg=dialogs.notify("通知","Session已经过期,请重新登录！", {'size': 'sm','backdrop':'static'}); dlg.result.then(function(btn){
				 * location.href="logout"; });
				 */
			});
			...
			$scope.app = {
				...
				settings : {
					...
					navbarCollapseColor : 'bg-light-blue',
					asideColor : 'bg-black',
					headerFixed : true,
					asideFixed : false,
					asideFolded : false,
			...
			/*
			 * // angular translate $scope.lang = { isopen: false }; // $scope.langs = {zh_CN:'简体中文', en:'English', de_DE:'German', it_IT:'Italian'};
			 * $scope.langs = {zh_CN:'简体中文'}; // $scope.selectLang = $scope.langs[$translate.proposedLanguage()] || "English"; $scope.selectLang =
			 * $scope.langs['zh_CN']; $scope.setLang = function(langKey, $event) { // set the current lang $scope.selectLang = $scope.langs[langKey]; //
			 * You can change the language during runtime $translate.use(langKey); $scope.lang.isopen = !$scope.lang.isopen; };
			 */
